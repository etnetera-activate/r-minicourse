...
processDataset <- function(df,min_treshold){
  ...
  df <- select(df, date, iso.3166.1.alpha.3.codes, province.state, value)
  names(df) <- c("date","country","province","value")
  df$one <- 1
...
cases.by.day %>% filter(country %in% c("CZE","DEU","ITA","SPA","GBR", "AUS", "POL","SWE")) %>%
ggplot(aes(x=daysSinceStart,y=value,))+geom_line(aes(col=country))+
  #scale_y_continuous(labels=comma, trans = "log10")+
  scale_y_continuous(labels=comma, limits=c(0,20000))+
  theme_bw()+
  labs(y="Confirmed cases",x=sprintf("Days since %d cases",TRESHOLD_CASES))

deaths.by.day %>% filter(country %in% c("CZE","DEU","ITA","SPA","GBR", "AUS", "POL","SWE")) %>%
  ggplot(aes(x=daysSinceStart,y=value,))+geom_line(aes(col=country))+
  #scale_y_continuous(labels=comma, trans = "log10")+
  scale_y_continuous(labels=comma, limits=c(0,10000))+
  theme_bw()+
  labs(y="Confirmed cases",x=sprintf("Days since %d cases",TRESHOLD_DEATHS))
